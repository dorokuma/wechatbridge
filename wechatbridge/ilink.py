...
class ILinkClient:
    ...
    async def send_media_message(
        ...
        The item should be a fully constructed media item dict (e.g. image_item
        or file_item with nested media). This is the media counterpart of
        send_message (which sends text only).

        Reference: hermes weixin.py sendmessage API
        """
        ...
        try:
            ...
            if ret != 0 and not message_id:
                logger.warning(
                    "send_media_message failed ret=%s for %s: %s",
                    ret, to_user_id, data,
                )
                return False
            if ret != 0 and message_id:
                logger.warning(
                    "send_media_message ret=%s but has message_id=%s, treating as success",
                    ret, message_id,
                )
            logger.info(
                "Media message sent to %s: msg_id=%s",
                to_user_id, data.get("message_id"),
            )
            return True
        except httpx.HTTPStatusError as e:
            if e.response.status_code in (401, 403):
                logger.warning(
                    "Bot token rejected during send_media_message (401/403), clearing state"
                )
                self.state.clear()
            else:
                logger.warning("HTTP error in send_media_message: %s", e)
            return False
        except httpx.RequestError as e:
            logger.warning("Network error in send_media_message: %s", e)
            return False
        except Exception as e:
            logger.exception("Unexpected error in send_media_message: %s", e)
            return False
    ...
    async def send_message(
        ...
        """
        Send a text message reply via iLink.
        context_token must come from the inbound message.
        """
        url = f"{baseurl}/ilink/bot/sendmessage"
        headers = _common_headers(has_auth=True, bot_token=bot_token)
        client_id = f"wechatbridge-{uuid.uuid4().hex[:16]}"
        ...
        try:
            ...
            if ret != 0 and not message_id:
                logger.warning(
                    "sendmessage failed ret=%s for %s: %s",
                    ret, to_user_id, data,
                )
                return False
            if ret != 0 and message_id:
                logger.warning(
                    "sendmessage ret=%s but has message_id=%s, treating as success",
                    ret, message_id,
                )
            logger.info(
                "Message sent to %s: msg_id=%s (%.50s)",
                to_user_id, data.get("message_id"), text,
            )
            return True
            ...
            if e.response.status_code in (401, 403):
                logger.warning(
                    "Bot token rejected during send (401/403), clearing state"
                )
                self.state.clear()
            else:
                logger.warning("HTTP error in send_message: %s", e)
            return False
        except httpx.RequestError as e:
            logger.warning("Network error in send_message: %s", e)
            return False
        except Exception as e:
            logger.exception("Unexpected error in send_message: %s", e)
            return False
