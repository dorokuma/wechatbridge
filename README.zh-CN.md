...
WeChatBridge 把微信机器人接到 [agy](#前置条件)（一个 agentic CLI），让你在微信对话里就能读文件、跑命令、抓网页，并把生成的文件收回来。
...
- **agy CLI** 已安装并登录（`agy` 在 `PATH`，或设 `AGY_BIN_PATH`）。agy 是一个 agentic CLI（Antigravity / Gemini CLI），在本地跑工具。
